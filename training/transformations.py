...
def create_trended_calculations(fixture_history_df: pd.DataFrame,
...
def create_ensemble_training_df(
    ...
    fixture_history_df = fixture_history_df.merge(
        pre_season_history_df[
            ["name", "position", "pre_season_model_score"]
        ],
        on=["name", "position"],
        how="left",
        validate="many_to_one",
    ...
    fixture_history_with_horizons_df = (
        fixture_history_with_horizons_df.merge(
            fixture_details_for_ensemble_df,
            on=["player_id", "season", "fixture_id", "target_gw"],
            how="left",
            validate="many_to_one",
        )
    )

    fixture_history_with_horizons_df["in_season_model_score"] = (
        in_season_model.predict_for_dataframe(
            fixture_history_with_horizons_df
        )
    )
